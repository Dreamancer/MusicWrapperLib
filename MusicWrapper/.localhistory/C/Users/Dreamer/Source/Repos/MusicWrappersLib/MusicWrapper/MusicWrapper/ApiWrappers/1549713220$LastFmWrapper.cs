...
namespace MusicWrapper.ApiWrappers
{
	public class LastFmWrapper : MusicApiWrapper, IUserLibraryWrapper
	{
		...
		public async Task<ApiResponse> GetLibraryArtists(string userName, int? limit = null, int? page = null)
		{
			throw new NotImplementedException();
		...
		{
			return await GetResponse($"?method=user.getartisttracks&user={userName}&artist={artist}&api_key={_appSecret}&format=json");
		}
	}
}
